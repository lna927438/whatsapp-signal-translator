...
export const providerMessages: Record<ProviderCode, string> = {
  ...
  provider_quota: '翻译服务的 OpenAI API 额度不足或已达到用量上限，请管理员检查 API 账单。本次未扣减字符。',
...
interface ProviderEnv { OPENAI_API_KEY: string; OPENAI_MODEL: string }
export interface ProviderReadiness { status: 'available' | 'attention' | 'unverified'; code?: ProviderCode; model: string; checkedAt: string; latencyMs: number; inferenceTested: false }
...
export function checkProviderReadiness(env: ProviderEnv): Promise<ProviderReadiness> {
  ...
  const pending = (async (): Promise<ProviderReadiness> => {
    ...
    const result = (status: ProviderReadiness['status'], code?: ProviderCode): ProviderReadiness => ({ status, code, model,
    ...
    try {
      ...
      return payload?.id === model ? result('available') : result('unverified', 'provider_model')
    } catch (error: any) {
