...
<template>
  <div class="modal-backdrop" @click.self="emit('close')">
    <section v-if="settings" class="panel settings-panel" role="dialog" aria-modal="true" aria-label="全局设置">
      ...
      <section v-show="tab === 'runtime'" class="settings-group"><h3>桌面运行</h3><label class="setup-toggle"><span>开机启动（Windows / macOS）</span><input v-model="settings.autoStart" type="checkbox" /></label><label class="setup-toggle"><span>启动后最小化</span><input v-model="settings.startMinimized" type="checkbox" /></label><label class="setup-toggle"><span>点击右上角 X 后在后台运行（推荐）</span><input v-model="settings.closeToTray" type="checkbox" /></label><p>默认关闭窗口不退出、不停止后台翻译；通过系统托盘菜单“退出 HelloDog”完全退出。关闭此选项后，X 将退出窗口。启动最小化将在下次启动生效。</p></section>
